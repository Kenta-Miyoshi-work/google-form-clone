...
import { FaCalendarDays, FaCheck, FaGlobe } from "react-icons/fa6";
      ...
      <CardHeader>
        ...
        <div className="mt-3 grid gap-2 text-sm sm:grid-cols-2">
          <div className="flex items-center gap-2 rounded-md bg-slate-50 px-3 py-2 text-slate-600">
            <FaGlobe style={{ color: settings.themeColor }} />
      ...
      <Card className="border-t-8" style={{ borderTopColor: settings.themeColor }}>
        <CardContent className="space-y-4 p-8 text-center">
          ...
          <div className="mx-auto max-w-md rounded-lg bg-slate-50 p-4 text-left text-sm text-slate-600">
            ...
            {settings.allowEditAfterSubmit && <Button className="mt-3" variant="outline" onClick={onEdit}>回答を編集する</Button>}
          </div>
        </CardContent>
      </Card>
    </main>
    ...
    <main className="mx-auto max-w-3xl p-4 md:p-8">
      <Card className="border-t-8" style={{ borderTopColor: settings.themeColor }}>
        ...
        <CardContent className="space-y-4">
          ...
          {settings.allowEditAfterSubmit ? <Button className="bg-purple-600 hover:bg-purple-700" onClick={onEdit}>前回の回答を編集する</Button> : <p className="text-sm text-slate-500">送信後の編集は許可されていません。</p>}
        </CardContent>
      </Card>
    </main>
  );
...
export function RespondentFormPage({ form, respondentEmail }) {
  return (
    <div className="min-h-screen bg-slate-100">
      ...
      <AnswerForm form={form} submitLabel="回答を送信" respondentEmail={respondentEmail} />
    </div>
  );
}
  ...
  return (
    <main className="mx-auto max-w-3xl space-y-4 p-4 md:p-8" style={{ backgroundColor: settings.backgroundColor }}>
      ...
      <div className="flex justify-between gap-2 pb-12">
        ...
        <Button style={{ backgroundColor: settings.themeColor }} onClick={goNext}>{currentSectionIndex >= sections.length - 1 ? buttonLabel : "次へ"}</Button>
      </div>
    </main>
  );
}
