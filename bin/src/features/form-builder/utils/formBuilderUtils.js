...
export function normalizeSettings(value) {
  ...
  const audienceTargets = Array.isArray(value?.audienceTargets) ? value.audienceTargets.filter((target) => typeof target === "string" && target) : [];
  const collaboratorIds = Array.isArray(value?.collaboratorIds) ? value.collaboratorIds.filter((target) => typeof target === "string" && target) : [];
...
export const supportedQuestionTypes = ["shortText", "radio", "checkbox", "file"];

export function coerceQuestionType(type) {
  if (type === "select") return "radio";
  return supportedQuestionTypes.includes(type) ? type : "shortText";
}
...
export function validateAuthoringForm(value) {
  ...
  (Array.isArray(value.questions) ? value.questions : []).forEach((question, index) => {
    ...
    if (["radio", "checkbox"].includes(question?.type) && (!Array.isArray(question.options) || question.options.length === 0)) errors.push(`${label}.options は1件以上必要です。`);
...
export function createQuestionDefaults(type = "shortText") {
  const normalizedType = coerceQuestionType(type);
...
export function normalizeQuestion(value = {}) {
  const type = coerceQuestionType(value.type);
...
export function createFormFromActionItem(item) {
  ...
  return {
    id: item?.id,
    reviewAnswers: item?.reviewAnswers ?? null,
